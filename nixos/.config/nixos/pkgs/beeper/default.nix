# Vendored from nixpkgs nixos-unstable (pkgs/by-name/be/beeper/package.nix, 2026-09-04).
# The main pin ships 4.2.547, whose Desktop API lacks reactions, read/unread,
# message delete and the /v1/ws event feed that beeper.nvim needs (added 4.2.557+).
# Bumping the pin would rebuild the world; overriding version+hash alone fails because
# 4.2.985+ AppImages need the magic-bytes patch below. Drop this file when the pin catches up.
{
  lib,
  stdenv,
  runCommand,
