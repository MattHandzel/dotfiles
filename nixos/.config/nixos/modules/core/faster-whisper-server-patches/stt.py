...
def decode_upload_audio(file_bytes: bytes) -> tuple[np.ndarray, float]:
    if len(file_bytes) == 0:
        return np.array([], dtype=np.float32), 0.0

    try:
        audio = decode_audio(BytesIO(file_bytes), sampling_rate=SAMPLES_PER_SECOND)
    ...
    return audio, audio.shape[0] / SAMPLES_PER_SECOND


def request_language_name(language: Language | None) -> str:
    if language is None:
        return ""
    return language.value


def empty_streaming_response() -> StreamingResponse:
...
def empty_transcription_response(
    ...
    *,
    ...
    if stream:
        return empty_streaming_response()

    match response_format:
        case ResponseFormat.TEXT:
            return Response("", media_type="text/plain")
        case ResponseFormat.JSON:
            return Response(CreateTranscriptionResponseJson(text="").model_dump_json(), media_type="application/json")
        case ResponseFormat.VERBOSE_JSON:
            payload = CreateTranscriptionResponseVerboseJson(
                task=task.value,
                language=request_language_name(language),
                duration=duration,
                text="",
                words=[] if word_timestamps else None,
                segments=[],
            )
            return Response(payload.model_dump_json(), media_type="application/json")
        case ResponseFormat.VTT:
...
def has_speech(audio: np.ndarray) -> bool:
    return len(get_speech_timestamps(audio, EMPTY_VAD_OPTIONS)) > 0


def auto_language_detection_will_fail(whisper, audio: np.ndarray) -> bool:
    features = whisper.feature_extractor(audio)
    return features.shape[-1] <= whisper.feature_extractor.nb_max_frames


def is_empty_language_detection_error(exc: ValueError) -> bool:
    return str(exc) == EMPTY_LANGUAGE_DETECTION_ERROR
...
def translate_file(
    ...
    upload_bytes = read_upload_bytes(file)
    if len(upload_bytes) == 0:
        logger.info("Received empty translation upload. Returning an empty response.")
        return empty_transcription_response(
            response_format,
            task=Task.TRANSLATE,
    ...
    audio, duration = decode_upload_audio(upload_bytes)
    if audio.size == 0:
        logger.info("Decoded empty translation audio. Returning an empty response.")
        return empty_transcription_response(
            response_format,
            task=Task.TRANSLATE,
            language=config.default_language,
    ...
    if vad_filter and not has_speech(audio):
        logger.info("Translation upload contains no speech after VAD. Returning an empty response.")
        return empty_transcription_response(
            response_format,
            task=Task.TRANSLATE,
            language=config.default_language,
    ...
    effective_language = config.default_language
    with model_manager.load_model(model) as whisper:
        if effective_language is None and auto_language_detection_will_fail(whisper, audio):
            logger.info("Translation upload is too short for language detection. Returning an empty response.")
            return empty_transcription_response(
                response_format,
                task=Task.TRANSLATE,
        ...
        try:
            ...
            if is_empty_language_detection_error(exc):
                logger.info("Translation upload produced no usable speech. Returning an empty response.")
                return empty_transcription_response(
                    response_format,
                    task=Task.TRANSLATE,
                    language=effective_language,
...
def transcribe_file(
    ...
    upload_bytes = read_upload_bytes(file)
    if len(upload_bytes) == 0:
        logger.info("Received empty transcription upload. Returning an empty response.")
        return empty_transcription_response(
            response_format,
    ...
    audio, duration = decode_upload_audio(upload_bytes)
    if audio.size == 0:
        logger.info("Decoded empty transcription audio. Returning an empty response.")
        return empty_transcription_response(
            response_format,
            task=Task.TRANSCRIBE,
            language=language,
    ...
    if vad_filter and not has_speech(audio):
        logger.info("Transcription upload contains no speech after VAD. Returning an empty response.")
        return empty_transcription_response(
            response_format,
            task=Task.TRANSCRIBE,
            language=language,
    ...
    with model_manager.load_model(model) as whisper:
        if language is None and auto_language_detection_will_fail(whisper, audio):
            logger.info("Transcription upload is too short for automatic language detection. Returning an empty response.")
            return empty_transcription_response(
                response_format,
                task=Task.TRANSCRIBE,
                language=language,
        ...
        try:
            ...
            if is_empty_language_detection_error(exc):
                logger.info("Transcription upload produced no usable speech. Returning an empty response.")
                return empty_transcription_response(
                    response_format,
                    task=Task.TRANSCRIBE,
                    language=language,
