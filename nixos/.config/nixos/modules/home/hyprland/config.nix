...
let
  ...
  makeStringToIncaseSensitiveRegex = str:
      let
  ...
  generateSignletonWindowRules = singleton:
  let 
  ...
  if builtins.hasAttr singleton appKeyboardShortcuts then
    [
  ...
  apply_function_to_content = {function, content}:
      builtins.filter (x: x!="") (lib.splitString "\n" (lib.concatMapStringsSep "\n" (window: lib.concatStringsSep "\n" (function window)) content));

  generated_floating_windowrule = apply_function_to_content {function = generateFloatingRules; content= floating_windows;};
  generated_singleton_windowrule = apply_function_to_content {function = generateSignletonWindowRules; content= singleton_windows;};
  generatedSingltonKeyboardShortcuts = apply_function_to_content {function = generateSingletonKeyboardShortcuts; content= singleton_windows;};
in
{
...
hide_on_key_press = true;
      ...
      bind = generatedSingltonKeyboardShortcuts ++ [
        # show keybinds list
      ...
      # windowrule
      windowrule = generated_floating_windowrule ++ [
      ...
      # windowrulev2
      windowrulev2 = generated_singleton_windowrule ++ [
