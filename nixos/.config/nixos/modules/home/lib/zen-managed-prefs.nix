# The nix-managed block of Zen Browser prefs, shared by the Linux and macOS
# variants of zen-config (the two differ only in where Zen keeps its profiles:
# ~/.zen on Linux, ~/Library/Application Support/zen/Profiles on macOS).
{pkgs}:
pkgs.writeText "zen-managed-prefs.js" ''
