...
if [ $? != 0 ]; then
	...
	tmux send-keys -t $SESSION_NAME "nvim your_note.txt" C-m
fi

# Launch your terminal and attach it to the tmux session
alacritty -e bash -c "echo -ne '\033]0;notetaker\007'; tmux attach -t $SESSION_NAME"
